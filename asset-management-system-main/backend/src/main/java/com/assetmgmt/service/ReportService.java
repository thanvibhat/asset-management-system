...
public class ReportService {
    ...
    public byte[] exportAllocationReport() {
        ...
        StringBuilder csv = new StringBuilder("Asset Tag,Asset Name,Assigned To,Allocated At,Expected Return,Status\n");
        for (Allocation a : allocations) {
            csv.append(a.getAsset().getAssetTag()).append(",")
               .append("\"").append(a.getAsset().getName()).append("\",")
    ...
    public byte[] exportMaintenanceCostReport() {
        ...
        StringBuilder csv = new StringBuilder("Asset Tag,Asset Name,Type,Cost,Vendor,Scheduled Date,Status\n");
        for (MaintenanceRecord r : records) {
            csv.append(r.getAsset().getAssetTag()).append(",")
               .append("\"").append(r.getAsset().getName()).append("\",")
    ...
    public byte[] exportAssetAgingReport() {
        List<Asset> assets = assetRepository.findAll();
        StringBuilder csv = new StringBuilder("Asset Tag,Name,Category,Purchase Date,Age (Days),Current Value,Status\n");
    ...
    private byte[] exportMetricsCsv(List<com.assetmgmt.dto.AssetMetricsDto> metrics, String title) {
        StringBuilder csv = new StringBuilder("Asset Tag,Name,Category,Maint. Cost,Repair Count,Maint. Ratio,Retention %,Age (Days)\n");
